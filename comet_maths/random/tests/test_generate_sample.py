...
class TestGenerateSample(unittest.TestCase):
    ...
    def test_generate_sample_systematic(self):
        ...
        npt.assert_equal(sample.shape,(1000,20))
        npt.assert_allclose(u_x1,np.std(sample,axis=0),rtol=0.1)
        npt.assert_allclose(np.ones((20,20)),np.corrcoef(sample,rowvar=False),atol=0.1)
        ...
        npt.assert_allclose(corr_5_syst,np.corrcoef(sample.reshape((1000,-1)),rowvar=False),atol=0.1)
    ...
    def test_generate_sample_random(self):
        ...
        npt.assert_allclose(u_x5,np.std(sample,axis=0),rtol=0.1)
        npt.assert_allclose(corr_5_rand,np.corrcoef(sample.reshape((10000,-1)),rowvar=False),atol=0.1)
    ...
    def test_correlate_sample_corr(self):
        ...
        npt.assert_allclose(np.zeros_like(u_x5),np.std(sample_corr[0]-sample_corr[1],axis=0),atol=0.1)
        npt.assert_allclose(x5,np.mean(sample_corr[0],axis=0),rtol=0.1, atol=0.05)
        npt.assert_allclose(corr_5_rand,np.corrcoef(sample_corr[0].reshape((10000,-1)),rowvar=False),atol=0.1)
